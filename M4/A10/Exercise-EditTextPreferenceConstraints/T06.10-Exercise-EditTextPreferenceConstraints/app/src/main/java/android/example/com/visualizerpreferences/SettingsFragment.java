...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        String message = "Please enter a number between 0.1 and 3.";
        Context context = getContext();
        Toast popupMessage = Toast.makeText(context, message, Toast.LENGTH_LONG);

        String keyValue = getString(R.string.pref_size_key);
        String preferenceKey = preference.getKey();

        if (preferenceKey.equals(keyValue)) {
            String str = (String) newValue;
            try {
                Float floatValue = Float.parseFloat(str);
                if (floatValue > 3 || floatValue < 0.1) {
                    popupMessage.show();
                    return false;
                }   
